...
float pos[] = {0, 0};
float motorSpeed = 500;
float motorMaxSpeed = 10;
float motorAccel = 0.999995;
...
void setup() {  
  ...
  on("setMaxSpeed", setMaxSpeed);
  on("setAccel", setAccel);
  on("servo", moveServo);
...
void goTo(float x, float y) {
  ...
  float originalMotorSpeed = motorSpeed;
  // unsigned long stepDuration = 200; // The time it takes to perform one step in microseconds
  unsigned long motor1StepInterval = motorSpeed / motor1Speed;
  unsigned long motor2StepInterval = motorSpeed / motor2Speed;
  ...
  while (abs(motor1Target - motor1Step) > EPSILON || abs(motor2Target - motor2Step) > EPSILON) {
    ...
    motorSpeed = motorSpeed * motorAccel;
    if (motorSpeed < motorMaxSpeed) {
      motorSpeed = motorMaxSpeed;
    }
    motor1StepInterval = motorSpeed / motor1Speed;
    motor2StepInterval = motorSpeed / motor2Speed;
  }
  motorSpeed = originalMotorSpeed;
...
void goAdd(float x, float y) {

  // Serial.println("START GOTO");

  // Set your target distances for each motor (in steps)
  float motor1Target = x;
  float motor2Target = y;
...
uint8_t goFor(uint8_t* payload, int length, uint8_t* reply) {
  float x = read_float(payload, 0);
  float y = read_float(payload, 4);
...
uint8_t setSpeed(uint8_t* payload, int length, uint8_t* reply) {
  motorSpeed = read_float(payload, 0);
  return 0;
}

uint8_t setMaxSpeed(uint8_t* payload, int length, uint8_t* reply) {
  motorMaxSpeed = read_float(payload, 0);
  return 0;
}

uint8_t setAccel(uint8_t* payload, int length, uint8_t* reply) {
  motorAccel = read_float(payload, 0);
  return 0;
}
