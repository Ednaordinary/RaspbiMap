...
from np_file import np_load, np_save
from remaplib import *
from remapplotlib import *

# Based on https://github.com/john2zy/IMU-Position-Tracking/
...
class IMUTracker:
    ...
    def attitudeTrack(self, data, init_list):
        ...
        while t < sample_number:
            ...
            Ra = [(acc_noise / (np.linalg.norm(at) + 0.01))**2 + (1 - g0 / (np.linalg.norm(at)+0.01))**2] * 3
            Rm = [mag_noise**2] * 3
            R = np.diag(Ra + Rm)
...
def flatten_trajectory(traj):
    return np.concatenate((traj[:, :2], np.zeros((traj.shape[0], 1))), axis=1)

def plot_trajectory():
    tracker = IMUTracker(sampling=10)
